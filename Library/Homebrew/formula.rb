...
class Formula
  ...
  # Derives specs from class ivars
  def ensure_specs_set
    ...
    unless @url.nil?
      @stable = SoftwareSpecification.new
      @stable.url(@url)
    ...
    if @head.kind_of? String
      url = @head
      @head = SoftwareSpecification.new
...
public
  ...
  def fetch
    ...
    # TODO teach download strategies to take a SoftwareSpecificaton
...
private
  ...
  class << self
    ...
    SoftwareSpecification::CHECKSUM_TYPES.each do |cksum|
      class_eval %Q{
        def #{cksum}(val=nil)
          unless val.nil?
            @stable ||= SoftwareSpecification.new
    ...
    def devel &block
      ...
      @devel ||= SoftwareSpecification.new
